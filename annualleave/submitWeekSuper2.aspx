...
Dim superDuper as Boolean
    ...
    Sub Page_Load
		...
		End If
		superDuper = False
		If emp_no = "1060" Or emp_no = "1065" Then
			superDuper = True
		End If
		Call GetRecords()
		If IsPostBack Then
	...
	Function GetRecords()
        conMISC.Open()   
		If superDuper Then
			strSQL = "SELECT vh.emp_no, vh.weekend_date, emp.supervisor FROM vachistory AS vh INNER JOIN employees AS emp ON vh.emp_no = emp.emp_no WHERE (vh.approved = @approved) AND (emp.supervisor = @super OR vh.emp_no = @tammy) ORDER BY vh.emp_no"
	...
	Function Approve_Super()
		...
		msgLbl.Text = msgLbl.Text & "oldTake: " & oldTake & ", newTake: " & newTake & ", "
		...
		While i <= 5 
			...
			If newTake > 0 Then
				...
				cmdInsert.ExecuteNonQuery()						 
				conMISC.Close()	
				msgLbl.Text = msgLbl.Text & "BeginDate: " & DateValue(startDate) & ", "
			End If
		End While
